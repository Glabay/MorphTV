...
final class WavHeader implements SeekMap {
    ...
    public long getDurationUs() {
        return ((this.dataSize / ((long) this.blockAlignment)) * C0649C.MICROS_PER_SECOND) / ((long) this.sampleRateHz);
    }

    public SeekPoints getSeekPoints(long j) {
        long constrainValue = Util.constrainValue((((((long) this.averageBytesPerSecond) * j) / C0649C.MICROS_PER_SECOND) / ((long) this.blockAlignment)) * ((long) this.blockAlignment), 0, this.dataSize - ((long) this.blockAlignment));
    ...
    public long getTimeUs(long j) {
        return (Math.max(0, j - this.dataStartPosition) * C0649C.MICROS_PER_SECOND) / ((long) this.averageBytesPerSecond);
    }
