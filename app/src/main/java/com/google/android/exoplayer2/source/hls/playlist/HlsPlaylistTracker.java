...
public final class HlsPlaylistTracker implements Callback<ParsingLoadable<HlsPlaylist>> {
    ...
    private long initialStartTimeUs = 1;
    ...
    private final class MediaPlaylistBundle implements Callback<ParsingLoadable<HlsPlaylist>>, Runnable {
        ...
        public boolean isSnapshotValid() {
            ...
            long max = Math.max(30000, C0649C.usToMs(this.playlistSnapshot.durationUs));
        ...
        private void processLoadedPlaylist(HlsMediaPlaylist hlsMediaPlaylist) {
            ...
            if (this.playlistSnapshot != hlsMediaPlaylist2) {
                ...
                } else if (((double) (elapsedRealtime - this.lastSnapshotChangeMs)) > ((double) C0649C.usToMs(this.playlistSnapshot.targetDurationUs)) * HlsPlaylistTracker.PLAYLIST_STUCK_TARGET_DURATION_COEFFICIENT) {
                    this.playlistError = new PlaylistStuckException(this.playlistUrl.url);
                    blacklistPlaylist();
                }
            }
            this.earliestNextLoadTimeMs = elapsedRealtime + C0649C.usToMs(this.playlistSnapshot != hlsMediaPlaylist2 ? this.playlistSnapshot.targetDurationUs : this.playlistSnapshot.targetDurationUs / 2);
