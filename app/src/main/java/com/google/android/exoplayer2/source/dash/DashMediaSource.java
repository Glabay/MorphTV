...
public final class DashMediaSource implements MediaSource {
    ...
    private static final class DashTimeline extends Timeline {
        ...
        public Period getPeriod(int i, Period period, boolean z) {
            ...
            return period.set(obj, num, 0, this.manifest.getPeriodDurationUs(i), C0649C.msToUs(this.manifest.getPeriod(i).startMs - this.manifest.getPeriod(0).startMs) - this.offsetInFirstPeriodUs);
        }
        ...
        public static PeriodSeekInfo createPeriodSeekInfo(com.google.android.exoplayer2.source.dash.manifest.Period period, long j) {
            ...
            while (i2 < size) {
                ...
                if (segmentCount == 0) {
                    ...
                    if (segmentCount != -1) {
                        j4 = (firstSegmentNum + ((long) segmentCount)) - 1;
                        j3 = Math.min(j3, index.getTimeUs(j4) + index.getDurationUs(j4, j2));
                    }
                    j4 = max;
                } else {
                    i3 = i2;
    ...
    private DashMediaSource(DashManifest dashManifest, Uri uri, com.google.android.exoplayer2.upstream.DataSource.Factory factory, Parser<? extends DashManifest> parser, com.google.android.exoplayer2.source.dash.DashChunkSource.Factory factory2, CompositeSequenceableLoaderFactory compositeSequenceableLoaderFactory, int i, long j, Handler handler, MediaSourceEventListener mediaSourceEventListener) {
        ...
        this.refreshManifestRunnable = new C07391();
        this.simulateManifestRefreshRunnable = new C07402();
    }
    ...
    void onDashManifestPublishTimeExpired(long j) {
        if (this.expiredManifestPublishTimeUs == C0649C.TIME_UNSET || this.expiredManifestPublishTimeUs < j) {
    ...
    private void processManifest(boolean z) {
        ...
        if (!dashMediaSource.manifest.dynamic || createPeriodSeekInfo2.isIndexExplicit) {
            ...
            j2 = Math.min((getNowUnixTimeUs() - C0649C.msToUs(dashMediaSource.manifest.availabilityStartTimeMs)) - C0649C.msToUs(dashMediaSource.manifest.getPeriod(i).startMs), j2);
            if (dashMediaSource.manifest.timeShiftBufferDepthMs != C0649C.TIME_UNSET) {
                long max;
                long msToUs = j2 - C0649C.msToUs(dashMediaSource.manifest.timeShiftBufferDepthMs);
                while (msToUs < 0 && i > 0) {
                    i--;
        ...
        if (dashMediaSource.manifest.dynamic) {
            ...
            if (j4 == -1) {
                j4 = dashMediaSource.manifest.suggestedPresentationDelayMs != C0649C.TIME_UNSET ? dashMediaSource.manifest.suggestedPresentationDelayMs : 30000;
            }
            j2 = j3 - C0649C.msToUs(j4);
            min = j2 < MIN_LIVE_DEFAULT_START_POSITION_US ? Math.min(MIN_LIVE_DEFAULT_START_POSITION_US, j3 / 2) : j2;
        } else {
            min = 0;
        }
        dashMediaSource.sourceListener.onSourceInfoRefreshed(dashMediaSource, new DashTimeline(dashMediaSource.manifest.availabilityStartTimeMs, (dashMediaSource.manifest.availabilityStartTimeMs + dashMediaSource.manifest.getPeriod(0).startMs) + C0649C.usToMs(j), dashMediaSource.firstPeriodId, j, j3, min, dashMediaSource.manifest), dashMediaSource.manifest);
    ...
    private long getNowUnixTimeUs() {
        if (this.elapsedRealtimeOffsetMs != 0) {
            return C0649C.msToUs(SystemClock.elapsedRealtime() + this.elapsedRealtimeOffsetMs);
        }
        return C0649C.msToUs(System.currentTimeMillis());
    }
}
