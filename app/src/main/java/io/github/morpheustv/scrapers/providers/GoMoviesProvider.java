...
public class GoMoviesProvider extends BaseProvider {
    String base_link;
    String[] domains;

    public GoMoviesProvider(Scraper scraper) {
        super(scraper, "GOMOVIES.SC", false);
        this.domains = new String[]{"gomovies.sc"};
        this.base_link = "https://www3.gomovies.sc";
        ...
        try {
            for (String str3 : list) {
                try {
                    ...
                    Iterator it = Jsoup.connect(stringBuilder.toString()).userAgent(UserAgent).validateTLSCertificates(false).timeout(5000).followRedirects(true).ignoreContentType(true).ignoreHttpErrors(true).get().select("div.ml-item").iterator();
                    while (it.hasNext()) {
                        Element element = (Element) it.next();
                        if (BaseProvider.cleantitle(element.select("h2").text()).equals(BaseProvider.cleantitle(str3))) {
                            attr = element.select("a.ml-mask").attr("href");
                            break;
                        }
                    }
                    attr = null;
            ...
            try {
                for (String str2 : list) {
                    try {
                        ...
                        stringBuilder.append("/");
                        Iterator it = Jsoup.connect(new URL(url, stringBuilder.toString()).toString()).userAgent(UserAgent).validateTLSCertificates(false).timeout(5000).followRedirects(true).ignoreContentType(true).ignoreHttpErrors(true).get().select("div.ml-item").iterator();
                        while (it.hasNext()) {
                            Element element = (Element) it.next();
    ...
    public void getSources(ProviderSearchResult providerSearchResult, CopyOnWriteArrayList<Source> copyOnWriteArrayList) {
        if (providerSearchResult == null) {
            ...
            Iterator it = Jsoup.connect(providerSearchResult2.getPageUrl()).userAgent(UserAgent).validateTLSCertificates(false).timeout(5000).followRedirects(true).ignoreContentType(true).ignoreHttpErrors(true).get().select("a.btn-eps").iterator();
            while (it.hasNext()) {
                Element element = (Element) it.next();
                try {
