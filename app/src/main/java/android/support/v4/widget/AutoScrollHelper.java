...
public abstract class AutoScrollHelper implements OnTouchListener {
    ...
    private static class ClampedScroller {
        ...
        private float getValueAt(long j) {
            ...
            if (this.mStopTime >= 0) {
                if (j >= this.mStopTime) {
                    return (1065353216 - this.mStopValue) + (this.mStopValue * AutoScrollHelper.constrain(((float) (j - this.mStopTime)) / ((float) this.mEffectiveRampDown), 0.0f, 1.0f));
                }
            }
            return AutoScrollHelper.constrain(((float) (j - this.mStartTime)) / ((float) this.mRampUpDuration), 0.0f, 1.0f) * 1056964608;
        }
        ...
        switch (this.mEdgeType) {
            ...
            case 1:
                if (f < f2) {
                    ...
                    return (this.mAnimating == null || this.mEdgeType != Float.MIN_VALUE) ? 0.0f : 1.0f;
                }
                break;
            case 2:
                if (f < 0.0f) {
