    ...
    public void seekTo(int i, long j) {
        ...
        if (i >= 0) {
            if (timeline.isEmpty() || i < timeline.getWindowCount()) {
                ...
                if (timeline.isEmpty()) {
                    this.maskingWindowPositionMs = j == C0649C.TIME_UNSET ? 0 : j;
                    this.maskingPeriodIndex = 0;
                } else {
                    long defaultPositionUs = j == C0649C.TIME_UNSET ? timeline.getWindow(i, this.window).getDefaultPositionUs() : C0649C.msToUs(j);
                    Pair periodPosition = timeline.getPeriodPosition(this.window, this.period, i, defaultPositionUs);
                    this.maskingWindowPositionMs = C0649C.usToMs(defaultPositionUs);
                    this.maskingPeriodIndex = ((Integer) periodPosition.first).intValue();
                }
                this.internalPlayer.seekTo(timeline, i, C0649C.msToUs(j));
...
*/
    ...
    public long getDuration() {
        ...
        return C0649C.usToMs(this.period.getAdDurationUs(mediaPeriodId.adGroupIndex, mediaPeriodId.adIndexInAdGroup));
    }
    ...
    public int getBufferedPercentage() {
        ...
        if (bufferedPosition == C0649C.TIME_UNSET) {
            return 0;
        }
    ...
    public long getContentPosition() {
        ...
        return this.period.getPositionInWindowMs() + C0649C.usToMs(this.playbackInfo.contentPositionUs);
    }
    ...
    private void handlePlaybackInfo(PlaybackInfo playbackInfo, int i, boolean z, int i2) {
        ...
        if (this.pendingOperationAcks == 0) {
            if (playbackInfo.startPositionUs == C0649C.TIME_UNSET) {
