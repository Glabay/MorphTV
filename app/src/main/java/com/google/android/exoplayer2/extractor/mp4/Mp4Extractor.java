...
public final class Mp4Extractor implements Extractor, SeekMap {
    ...
    public static final ExtractorsFactory FACTORY = new C06891();
    ...
    static class C06891 implements ExtractorsFactory {
        C06891() {
        }
    ...
    private void processMoovAtom(ContainerAtom containerAtom) throws ParserException {
        ...
        for (int i2 = 0; i2 < containerAtom2.containerChildren.size(); i2++) {
            ...
            if (containerAtom3.type == Atom.TYPE_trak) {
                Track parseTrak = AtomParsers.parseTrak(containerAtom3, containerAtom2.getLeafAtomOfType(Atom.TYPE_mvhd), C0649C.TIME_UNSET, null, (mp4Extractor.flags & 1) != 0, mp4Extractor.isQuickTime);
                if (parseTrak != null) {
                    ...
                    if (parseStbl.sampleCount != 0) {
                        ...
                        long max = Math.max(j, parseTrak.durationUs != C0649C.TIME_UNSET ? parseTrak.durationUs : parseStbl.durationUs);
                        if (parseTrak.type == 2) {
                            if (i == -1) {
                                i = arrayList.size();
                            }
