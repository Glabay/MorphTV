...
public class Series9Provider extends BaseProvider {
    String base_link;
    String[] domains;
    String search_link;

    public Series9Provider(Scraper scraper) {
        super(scraper, "SERIES9.IO", true);
        this.domains = new String[]{"series9.io"};
        this.base_link = "https://www2.series9.io";
        ...
        try {
            for (String str3 : list) {
                try {
                    ...
                    stringBuilder2.append("/series//movie/search/");
                    ...
                    Iterator it = Jsoup.parse(wvgethtml(stringBuilder3)).select("div.ml-item").iterator();
                    while (it.hasNext()) {
                        Element element = (Element) it.next();
                        if (BaseProvider.cleantitle(element.select("h2").text()).equals(BaseProvider.cleantitle(str3))) {
                            String attr = ((Element) element.select("a").get(0)).attr("data-url");
                            StringBuilder stringBuilder4 = new StringBuilder();
                            stringBuilder4.append(attr);
                            stringBuilder4.append("?link_web=");
                            stringBuilder4.append(this.base_link);
                            stringBuilder4.append("/");
                            Document parse = Jsoup.parse(StringEscapeUtils.unescapeHtml4(StringEscapeUtils.unescapeEcmaScript(Jsoup.connect(stringBuilder4.toString()).validateTLSCertificates(false).ignoreHttpErrors(true).ignoreContentType(true).userAgent(UserAgent).referrer(stringBuilder3).timeout(10000).execute().body())));
                            if (((Element) parse.select("div.jt-info").get(1)).text().equals(str)) {
                                stringBuilder3 = parse.select("a.btn-success").attr("href");
                                break;
                            }
                        }
                    }
                    ...
                    if (stringBuilder3 != null) {
                        ...
                        providerSearchResult.setPageUrl(stringBuilder3);
