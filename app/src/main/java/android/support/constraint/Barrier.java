...
public class Barrier extends ConstraintHelper {
    ...
    protected void init(AttributeSet attributeSet) {
        ...
        if (attributeSet != null) {
            attributeSet = getContext().obtainStyledAttributes(attributeSet, C0021R.styleable.ConstraintLayout_Layout);
            int indexCount = attributeSet.getIndexCount();
            for (int i = 0; i < indexCount; i++) {
                int index = attributeSet.getIndex(i);
                if (index == C0021R.styleable.ConstraintLayout_Layout_barrierDirection) {
                    setType(attributeSet.getInt(index, 0));
                } else if (index == C0021R.styleable.ConstraintLayout_Layout_barrierAllowsGoneWidgets) {
