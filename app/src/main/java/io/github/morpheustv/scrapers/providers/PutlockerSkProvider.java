...
public class PutlockerSkProvider extends BaseProvider {
    String base_link;
    String[] domains;

    public PutlockerSkProvider(Scraper scraper) {
        super(scraper, "PUTLOCKERTV.TO", true);
        this.domains = new String[]{"www6.putlockertv.to"};
        this.base_link = "https://www6.putlockertv.to";
        this.resourceWhitelist = new String[]{"(.*chk_jschl?.*)", "(.*jquery.*)", "(.*public/all.*.js.*)", "(.*episode/info.*)", "(.*search?keyword=.*)", "(.*/ajax/.*)"};
    }
        ...
        try {
            ...
            for (String str3 : list) {
                try {
                    ...
                    stringBuilder2.append("/search?keyword=");
                    stringBuilder2.append(cleantitlequery(str3));
                    Iterator it = Jsoup.parse(wvgethtml(new URL(url, stringBuilder2.toString()).toString())).select("div.film-list").select("div.inner").iterator();
                    while (it.hasNext()) {
                        ...
                        if (BaseProvider.cleantitle(((Element) element.select("a.name").get(0)).text()).equals(BaseProvider.cleantitle(str3))) {
                            ...
                            Document document = Jsoup.connect(stringBuilder2.toString()).ignoreHttpErrors(true).validateTLSCertificates(false).ignoreContentType(true).header(HttpHeaders.COOKIE, cookie).userAgent(UserAgent).timeout(10000).get();
                            if (document.select("div.title").select(TtmlNode.TAG_SPAN).text().equals(str)) {
                                StringBuilder stringBuilder3 = new StringBuilder();
                                stringBuilder3.append(this.base_link);
                                stringBuilder3.append(document.select("a.watch-now").attr("href"));
                ...
                for (String str2 : list) {
                    try {
                        ...
                        if (i > 1) {
                            ...
                            url2 = new URL(url, stringBuilder.toString()).toString();
                        }
                        Iterator it = Jsoup.parse(wvgethtml(url2)).select("div.film-list").select("div.inner").iterator();
                        while (it.hasNext()) {
                            Element element = (Element) it.next();
                            ...
                            if (!cleantitle.equals(BaseProvider.cleantitle(stringBuilder3.toString()))) {
                                if (BaseProvider.cleantitle(text).equals(BaseProvider.cleantitle(str2))) {
                                    ...
                                    stringBuilder4.append(element.select("a.poster").attr("href"));
                        ...
                        if (url2 != null) {
                            ...
                            providerSearchResult2.setPageUrl(url2);
