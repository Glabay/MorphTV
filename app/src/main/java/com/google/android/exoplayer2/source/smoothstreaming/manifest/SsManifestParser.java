...
public class SsManifestParser implements Parser<SsManifest> {
    ...
    private static class StreamIndexParser extends ElementParser {
        ...
        private void parseStreamFragmentStartTag(XmlPullParser xmlPullParser) throws ParserException {
            ...
            long parseLong = parseLong(xmlPullParser, KEY_FRAGMENT_START_TIME, C0649C.TIME_UNSET);
            int i = 1;
            if (parseLong == C0649C.TIME_UNSET) {
                if (size == 0) {
                    parseLong = 0;
            ...
            this.lastChunkDuration = parseLong(xmlPullParser, KEY_FRAGMENT_DURATION, C0649C.TIME_UNSET);
            long parseLong2 = parseLong(xmlPullParser, "r", 1);
            if (parseLong2 <= 1 || this.lastChunkDuration != C0649C.TIME_UNSET) {
                while (true) {
                    long j = (long) i;
                    if (j < parseLong2) {
